...
## 설정 4 — Storage(사진 저장소) 만들기

현장 사진은 Firebase **Storage**에 파일로 올리고, 문서에는 사진 주소(URL)만 저장합니다.
(예전처럼 사진을 문서 안에 통째로 넣지 않으므로 문서가 가벼워지고 사진을 여러 장 넣어도 느려지지 않습니다.)

1. 왼쪽 메뉴 **빌드 → Storage → 시작하기**
   - 최근 만든 프로젝트는 Storage를 켤 때 **요금제 업그레이드(Blaze, 종량제)**를 요구할 수 있습니다.
     Blaze도 **매월 무료 제공량(저장 5GB·하루 다운로드 1GB)**이 넉넉해 이 정도 사용량은 **사실상 무료**지만,
     결제(카드) 계정 등록이 필요합니다. 위치는 `asia-northeast3 (서울)` 권장.
2. **규칙(Rules)** 탭에서 아래로 교체 후 **게시**:
   ```
   rules_version = '2';
...
> Storage를 켜지 않으면 사진 첨부 시 "Storage가 아직 설정되지 않았습니다" 안내가 뜹니다. 나머지 기능은 그대로 동작합니다.
...
브라우저가 사진을 대시보드용 크기로 자동 압축한 뒤 **Firebase Storage에 업로드**하고, 문서에는 사진 주소(URL)만 저장합니다.
따라서 문서가 가벼워 사진을 여러 장 넣어도 느려지지 않습니다. (사진 기능을 쓰려면 위 **설정 4 — Storage**가 먼저 되어 있어야 합니다.)
